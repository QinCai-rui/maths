...
export function registerCollaborativeSetServer(io: AnyServer): { flush: () => void } {
  ...
  namespace.on("connection", (socket) => {
    ...
    socket.on("applyClientHistory", (rawInput, ack) => {
      const parsed = ApplyClientHistoryInput.safeParse(rawInput);
      if (!parsed.success) return reject(ack, "Invalid client history operation");
      const session = activeSession(socket, ack);
      if (!session) return;
      const { target, expected, desired } = parsed.data;
      if (target.type !== "question" && !socket.data.isHost) {
        return reject(ack, "Only the host can apply this operation");
      }
      if (target.type !== "details" && lockedByOther(socket, [target.questionId])) {
        return reject(ack, "The question is locked by another collaborator");
      }
      if (!targetEqual(session.set, expected, target)) {
        return reject(ack, "Cannot apply this operation: this section has changed");
      }
      applyTarget(session.set, desired, target);
      if (target.type === "structure" && questionIndex(session.set, target.questionId) < 0) {
        locks.get(session.set.sessionToken)?.delete(target.questionId);
        namespace.to(roomName(session.set.sessionToken)).emit("lockChanged", target.questionId, null);
      }
      saveAndEmit(session);
      accept(ack);
    });
