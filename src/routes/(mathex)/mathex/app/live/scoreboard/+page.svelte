<script lang="ts">
  ...
  import { ArrowLeft, Radio, UsersRound } from "@lucide/svelte/icons";
  ...
  async function setCode(value: string) {
    ...
    if (await socket.emitWithAck("checkCompetition", code)) {
      socket.disconnect();
      goto(`/mathex/app/live/scoreboard/${code}`);
    } else {
      lastCode = "";
...
<svelte:head><title>Scoreboard - Mathex Live</title></svelte:head>
  ...
  <main class="relative w-full max-w-lg">
    ...
    <section class="mathex-panel rounded-3xl p-6 sm:p-9">
      <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"
        ><Radio class="h-6 w-6" /></span
      >
      <p class="mathex-kicker mt-7">Public display</p>
      <Header size="h1" class="mt-2 text-4xl tracking-[-0.04em]">Open the scoreboard.</Header>
      <p class="mt-3 leading-7 text-muted-foreground">
        Enter the six-digit live competition code to show its projector-safe standings.
      </p>
      ...
      <div class="mt-6 flex items-center gap-2 text-sm text-muted-foreground">
        <UsersRound class="h-4 w-4 text-primary" /> No PIN is required for the public scoreboard.
      </div>
    </section>
  </main>
</div>
